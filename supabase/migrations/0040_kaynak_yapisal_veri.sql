-- =============================================================================
-- Düğünce · 0040 · admin_create_venue: kapasite, fiyat, iç/dış mekan, özellik
--
-- Kaynak (dugun.com dökümü gibi) yalnızca ad/telefon/adres değil; kapasite,
-- fiyat aralığı, iç/dış mekan ve özellik (otopark, vale, alkol servisi...)
-- gibi YAPILANDIRILMIŞ olguları da veriyor. Bunları elle wizard'dan tekrar
-- girmek yerine katalog kaydı açılırken tek işlemde yazıyoruz.
--
-- BİLEREK YAZILMAYAN alan: description. Kaynağın pazarlama metni mekan
-- sahibinin KENDİ anlatımı değil; description hâlâ yalnızca wizard'dan
-- gelir ve venue_completion_of() onu ölçüyor (bkz. CLAUDE.md). Buradaki
-- yapılandırılmış olgular yerine venue_auto_summary() (0030/0031) zaten
-- description boşken devreye giriyor — kapasite/özellik dolunca otomatik
-- özet de kendiliğinden zenginleşiyor.
--
-- Özellik eşlemesi JS katmanında yapılıyor (lib/import/dugun-com.ts); burada
-- yalnızca gelen slug'lar `features` tablosunda var mı diye bakılıyor —
-- bilinmeyen/pasif slug sessizce atlanıyor, hata vermiyor.
--
-- `price_max`: mevcut `starting_price` tekil bir sayı (asgari/"…'den
-- başlayan"); kaynak çoğu zaman bir ARALIK veriyor (ör. 75.000–95.000 ₺).
-- Üst sınırı da ayrı kolonda tutuyoruz — `price_note` serbest metin, yapısal
-- sorgu/gösterim için değil.
-- =============================================================================
...
grant execute on function public.admin_create_venue(
  text, uuid, uuid, uuid, uuid, text, text, text, boolean, text, text,
  numeric, numeric, numeric, integer, text, text,
  integer, integer, numeric, numeric, text, text, boolean, boolean, text[]) to authenticated;
