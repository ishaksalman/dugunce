-- =============================================================================
-- Düğünce · 0044 · admin_create_venue: instagram_url
--
-- `venues.instagram_url` 0001'den beri şemada duruyordu ama
-- `admin_create_venue` hiç yazmıyordu — kaynak (dugun.com) artık gerçek bir
-- Instagram adresi verebiliyor (bkz. kazıyıcıdaki resolveInstagramUrl),
-- JS katmanı (lib/import/dugun-com.ts) da bunu taşıyordu ama SQL tarafında
-- inecek bir yer yoktu; sessizce atılıyordu.
--
-- EN SON sürümden (0040) türetildi — tek değişiklik p_instagram_url.
-- =============================================================================
...
drop function if exists public.admin_create_venue(
  ...
  integer, integer, numeric, numeric, text, text, boolean, boolean, text[]);
...
grant execute on function public.admin_create_venue(
  text, uuid, uuid, uuid, uuid, text, text, text, boolean, text, text,
  numeric, numeric, numeric, integer, text, text,
  integer, integer, numeric, numeric, text, text, boolean, boolean, text[], text) to authenticated;
